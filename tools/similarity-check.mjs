...
const RUBRIC = [
  ...
  // ---- 画面 (35) ----
  ["画面", "第三人称追尾透视赛道(3D 感)", 6, () => P(S([/drawGround/, /GFOCAL/]), 0.85, "Mode-7 伪 3D，非真 3D 引擎")],
  ["画面", "Q版卡通卡丁车+头盔车手", 5, () => P(S([/renderKartFrame/, /头盔/]), 0.9, "原创造型规避版权素材")],
  ["画面", "多主题赛道(村庄/雪谷/沙城)", 4, () => {
  ...
  ["画面", "氮气火焰喷射+速度线特效", 3, () => S([/kind: "flame"/, /drawSpeedFX/])],
  ["画面", "天空/太阳/云/远山视差背景", 3, () => S([/drawSky/, /视差/])],
  ["画面", "路旁装饰物(树木/路牌/起点拱门)", 3, () => S([/decorSprites/, /gate/])],
  ["画面", "车身动态(漂移倾斜/高速颠簸/撞墙抖动)", 2, () => S([/漂移车身倾斜/, /bob/])],
  ["画面", "起跑信号灯+倒计时缩放动画", 2, () => S([/drawCountdown/, /信号灯/])],
  // ---- 系统与HUD (20) ----
